...
## CRITICAL: OpenAI SDK & gpt-4o-mini gotchas

The `openai` npm package v6+ has TWO different APIs. **gpt-4o-mini does NOT work with the Chat Completions API** — it returns empty content. You MUST use the **Responses API** instead.

### Use the Responses API (NOT Chat Completions)

```typescript
// CORRECT — Responses API (works with gpt-4o-mini)
...
// WRONG — Chat Completions API (returns empty content with gpt-4o-mini)
...
- `max_tokens` is NOT supported by gpt-4o-mini— use `max_completion_tokens` if using Chat Completions (but don't use Chat Completions, use Responses API)
